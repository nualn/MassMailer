...
class Tbl:
    def __init__(self, root, state):
        ...
        edit_button = ttk.Button(
            frame, text="Update ", command=self.update_row)
        edit_button.grid(row=2, column=1)

        clear_button = ttk.Button(
            frame, text="Clear ", command=self.clear_fields)
    ...
    def update_row(self):
        selected = self.tbl.focus()
        # save new data
        self.tbl.item(selected, text="", values=list(
            map(lambda entry: entry.get(), self._entry_fields)))
        self.clear_fields()

    def insert_row(self):
        self.tbl.insert(parent='', index='end', values=list(
            map(lambda entry: entry.get(), self._entry_fields)))
        self.clear_fields()
